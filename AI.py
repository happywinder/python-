...
def get_line_score(line):
    ...
    for i in line:
        ...
        if not tt[1]:
            for size in ['002221', '122200', '020221', '122020', '022021', '120220', '20022', '22002', '20202',
                         '1022201']:
                if i.find(size) != -1:
                    tt[1] = 1
                break
...
def get_score(pos, board):
    ...
    # 左斜
    lx = str([ori[i][i - pos[0] + pos[1]] for i in range(15) if (i - pos[0] + pos[1]) in range(15)])[1:-1].replace(',',
                                                                                                                   '').replace(
        ' ', '')

    # 右斜
    rx = str([ori[i][pos[0] + pos[1] - i] for i in range(15) if (pos[0] + pos[1] - i) in range(15)])[1:-1].replace(',',
                                                                                                                   '').replace(
        ' ', '')
